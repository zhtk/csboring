...
let test n b =
  ...
  else Printf.printf "Dobry wynik testu %d!!\n" n
...
a := add (1,1) !a;;
a := add (2,2) !a;;
test 4 (first (split 1 !a) = empty);;
test 5 (second (split 1 !a));;
test 6 (not (second (split 3 !a)));;
test 7 (third (split 3 !a) = empty);;
a := add (3,3) !a;;
test 8 (mem 1 !a);;
test 9 (mem 2 !a);;
test 10 (mem 3 !a);;
...
test 32 (first (split min_int !b) = empty);;
...
test 156 (first q = empty && not (second q) && third q = empty);;
